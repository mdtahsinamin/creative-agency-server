...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.ntqwp.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.post("/clientEnrollCourse", (req, res) => {
      ...
      customerCollection
        .insertOne({ name, email, course, projectDetails, status, image })
        .then((result) => {
          res.send(result.insertedCount > 0);
        });
    });
    ...
    app.post("/isAdmin", (req, res) => {
      ...
      adminCollection
        .find({ email: req.body.loginUserEmail })
        .toArray((err, Admins) => {
